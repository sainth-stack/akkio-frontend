...
const PredictData = () => {
  ...
  const datann = [
    ...
    {
      id: "36% ",
      name: " OKRDATA1707843206710.xlsx ",
      freq: "60%",
      freq2: "75%",
    },
  ...
  const handleGetDataFinalData = async (id) => {
    const response = await axios.get(
      `${akkiourl}/predict/${db}/${id.replace(" ", "_")}`
    );
    if (response.status === 200) {
      const data = response?.data?.data;
      const finSampleData = transformData(JSON.parse(data?.sample_rows)).slice(
        0,
        15
      );
      setTotData({
  ...
  const { displayContent } = useDataAPI();
        ...
        <div className="col-md-3">
          <div className="predictLeftCont">
            ...
            <p className="paragraphText">
              Select which numerical or categorical fields to predict and
              optionally ignore.
            </p>
              ...
              <div style={{ alignItems: "center" }}>
                <div className="px-2">
                  <input
                    ...
                    class="prediction-multiselect-searchbox"
                  />
                </div>
                <div
                  style={{
                    ...
                    return (
                      ...
                      >
                        <div
                          className={
                            selectedField === item
                              ? "checkboxContainer checkboxTick"
                              : "checkboxContainer"
                          }
                        ></div>
        ...
        <div className="col-md-9 rightContainer">
          <div className="rightttconttt">
            <div className="rightHeaderText">
              <img src={regression} alt="imag" /> Regression Summary
            </div>
            <h2 className="rightDesctext">
              Below is a breakdown of how well the model predicted your
              prediction column.
            </h2>
            <div
              className="d-flex"
              style={{ gap: "10px", marginRight: "50px" }}
            >
              <div className="regressioncard">
                <div className="regressionCardInnerContainer">
                  <p className="cardFirstLineText">
                    {totData?.metrics || "metrics"} is usually within
                  </p>
                  <p className="cardLargeNumberText">
                    {" "}
                    ±{(totData?.accuracy).toFixed(1)}%{" "}
                  </p>
                  <p className="cardFirstLineText">
                    {" "}
                    Predicted values were off by{" "}
                  </p>
                </div>
              </div>
              <div className="regressioncard">
                <div className="row justify-content-center align-items-center">
                  ...
                  <div className="regressionCardInnerContainer col-5">
                    <p className="secondcardFirstLineText">
                      Average Transaction Successfull
                    </p>
                    <p className="secondcardLargeNumberText"> 65% </p>
                    <p className="secondcardFirstLineText">
                      {" "}
                      Average Transaction Failed{" "}
                    </p>
            ...
            <div>
              <div className="rightHeaderText">
                <img src={topFactors} alt="imag" /> Top Fields
              </div>
              <h2 className="rightDesctext">
                {" "}
                Individual factors ranked by their contribution to the
                prediction results{" "}
              </h2>
                ...
                <div className="regressionCardInnerContainer">
                  <table className="datasetTable">
                    <thead className="datasetHeader">
                      <tr>
                        {Object.values(totData?.topFields["Features"])?.map(
                          (item, index) => {
                            return <th>{item}</th>;
                          }
                        )}
                    ...
                    <tbody className="datasetBody">
                      <tr>
                        {Object.values(totData?.topFields["Importances"])?.map(
                          (item, index) => {
                            return <td>{item}</td>;
                          }
                        )}
            ...
            <div>
              <div className="rightHeaderText">
                <img src={sampleRows} alt="imag" /> Sample Rows
              </div>
              <h2 className="rightDesctext">
                {" "}
                Sample rows of your data sorted by predicted value of the
                outcome of interest. Drag the slider to inspect rows at
                different values.{" "}
              </h2>
              <div className="regressioncard" style={{ width: "100%" }}>
                <div className="regressionCardInnerContainer">
                  <table className="datasetTable">
                    <thead className="datasetHeader">
                      <tr>
                        {Object.keys(totData?.finSamplerows[0])?.map((item) => {
                          return <th>{item}</th>;
                        })}
                      </tr>
                    </thead>
                    <tbody className="datasetBody">
                      {totData?.finSamplerows?.map((row, index) => {
                        if (
                          row[Object.keys(totData?.finSamplerows[0])[0]] !==
                            undefined ||
                          row[Object.keys(totData?.finSamplerows[0])[1]] !==
                            undefined
                        ) {
                          return (
                            <tr>
                              {newSamplerowsCols(
                                Object.keys(totData?.finSamplerows[0])
                              )?.map((col, key) => {
                                return (
                                  <td
                                    key={key}
